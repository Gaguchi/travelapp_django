<?php include 'header.php'; ?>
        <!-- PAGE TITLE
        ================================================== -->
        <section class="page-title-section bg-img cover-background top-position1" data-overlay-dark="5" data-background="img/blog/blog-06.jpg">
            <div class="container">
                <div class="row">
                    <div class="col-md-12">
                        <h1>Demand & Capacity Planning</h1>
                        <ul>
                            <li><a href="index.php">Home</a></li>
                            <li>Demand & Capacity Planning</li>
                        </ul>                    
                    </div>
                </div>
            </div>
            <span class="triangle-3 z-index-1 d-none d-sm-inline-block"></span>
            <span class="triangle-4 z-index-1 d-none d-sm-inline-block"></span>
            <img src="img/content/dots2.png" class="position-absolute right-5 bottom-5 ani-top-bottom z-index-3 d-none d-sm-block" alt="...">
            <div class="page-title-round ani-move"></div>
        </section>

        <!-- Demand & Capacity Planning
        ================================================== -->
        <section>
            <div class="container project-info">
                <div class="row">
                    <div class="col-xl-4 col-lg-5 order-2 order-lg-1 order-md-2">
                        <div class="service-details-sidebar pe-lg-1-6 pe-xl-1-9">
                            <aside class="widget widget-nav-menu wow fadeIn mb-1-6" data-wow-delay="100ms">
                                <div class="widget-title">
                                    <h4 class="title h6 mb-0">Our Projects</h4>
                                </div>
                                <div class="widget-body">
                                    <ul class="list-style4">
                                        <li>
                                            <a href="work-force-planning.php">Work Force Planning</a>
                                        </li>
                                        <li>
                                            <a href="etl.php">ETL Processes</a>
                                        </li>
                                        <li>
                                            <a href="classification-of-documents.php">Classification of Documents</a>
                                        </li>
                                        <li>
                                            <a href="web-design.php">Wed Dev & Web Scraping</a>
                                        </li>
                                        <li>
                                            <a href="production-cost-optimisation-model.php">Production Cost Optimisation</a>
                                        </li>
                                        <li class="active">
                                            <a href="demand-and-capacity-planning.php">Demand & Capacity Planning</a>
                                        </li>
                                    </ul>
                                </div>
                            </aside>
                            <aside class="widget widget-address wow fadeIn p-0 mb-0" data-wow-delay="200ms">
                                <div class="cover-background bg-img p-1-9 rounded word-break dark-overlay" data-overlay-dark="6" data-background="img/content/sidebar-image.jpg">
                                    <div class="d-table-cell vertical-align-middle z-index-2 position-relative">
                                        <div class="d-flex mb-4">
                                            <div class="flex-shrink-0">
                                                <div class="contact-icons"><i class="ti-mobile"></i></div>
                                            </div>
                                            <div class="flex-grow-1 ms-3 ms-lg-4">
                                                <h4 class="h5 mb-1 text-white">Call Us</h4>
                                                <p class="mb-0 text-white"><a class="text-white" href="tel:02035764931">0203 576 4931</a></p>
                                            </div>
                                        </div>
                                        <div class="d-flex">
                                            <div class="flex-shrink-0">
                                                <div class="contact-icons"><i class="ti-email"></i></div>
                                            </div>
                                            <div class="flex-grow-1 ms-3 ms-lg-4">
                                                <h4 class="h5 mb-1 text-white">Email</h4>
                                                <p class="mb-0 text-white"><a class="text-white" href="mailto:info@mutabalis.com">info@mutabalis.com</a></p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </aside>
                        </div>
                    </div>
                    <div class="col-xl-8 col-lg-7 order-1 order-lg-2 mb-2-6 mb-lg-0 order-md-1">
                        <div class="detail-services">
                            <div class="mb-1-6 wow fadeIn" data-wow-delay="200ms">
                                <h3 class="h2 mb-3">Demand & Capacity Planning</h3>
                                <p class="w-95">
                                    In a recent project focused on demand and capacity planning for London hospitals, our team successfully developed and reported from a robust demand and capacity model. This intricate model empowered accurate reporting and forecasting of bed and attendance requirements across regions, sites, and clinical specialties. 
                                </p>
                                <p class="mb-0 w-95">Covering over 2.5 million patients and their demographic subsets, the model provided insights into future demands for the next decade. This strategic approach ensures hospitals can proactively address patient needs, optimize resource allocation, and enhance overall operational efficiency.</p>
                            </div>
                            
                            <div class="row mb-1-9 mt-n1-9">
                                <div class="col-sm-4 mt-1-9 wow fadeIn" data-wow-delay="100ms">
                                    <div class="card-style9">
                                        <div class="card-body">
                                            <span class="big-text">01</span>
                                            <div class="icon-box"><img src="/img/icons/icon-capacity.png" alt="..."></div>
                                            <h3 class="position-relative pb-3 mb-4 project-card-header text-white">Demand and Capacity Modeling</h3>
                                            <p class="mb-0 text-white opacity7 project-card-text">Our team developed a sophisticated demand and capacity model tailored for London hospitals, ensuring precise reporting and forecasting capabilities.</p>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-sm-4 mt-1-9 wow fadeIn" data-wow-delay="150ms">
                                    <div class="card-style9">
                                        <div class="card-body">
                                            <span class="big-text">02</span>
                                            <div class="icon-box"><img src="/img/icons/icon-forecast.png" alt="..."></div>
                                            <h3 class="position-relative pb-3 mb-4 project-card-header text-white">Data Reporting and Forecasting</h3>
                                            <p class="mb-0 text-white opacity7 project-card-text">The model enabled detailed reporting and forecasting of bed and attendance requirements, offering insights by region, site, and clinical specialty.</p>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-sm-4 mt-1-9 wow fadeIn" data-wow-delay="200ms">
                                    <div class="card-style9">
                                        <div class="card-body">
                                            <span class="big-text">03</span>
                                            <div class="icon-box"><img src="/img/icons/icon-08.png" alt="..."></div>
                                            <h3 class="position-relative pb-3 mb-4 project-card-header text-white">Demographic Subset Analysis</h3>
                                            <p class="mb-0 text-white opacity7 project-card-text">Covering over 2.5 million patients, our model provided a granular breakdown of demographic subsets, facilitating targeted planning for diverse patient needs.</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="mb-0">
                                <h3 class="mb-3">Approach to Future Projects</h3>
                                <p class="w-95 mb-1-9">For future demand and capacity planning projects, the approach begins with a comprehensive scope evaluation. We will refine modeling techniques, incorporating the latest advancements for increased accuracy and granularity. Client collaboration remains pivotal, ensuring the model aligns closely with evolving requirements. Additionally, focus extends to expanding capabilities in demographic subset analysis for more targeted insights.</p>
                                <div class="row mt-n1-9">
                                    <div class="col-md-6 mt-1-9">
                                        <div class="wow fadeIn steps-left" data-wow-delay="100ms" style="background-image: url(img/blog/blog-06.jpg);">
                                        </div>
                                    </div>
                                    <div class="col-md-6 mt-1-9">
                                        <ul class="list-unstyled ps-md-2 mb-0 wow fadeIn" data-wow-delay="200ms">
                                            <li class="mb-1-6">
                                                <span class="text-primary text-uppercase font-weight-600 small">Step 1</span>
                                                <h3 class="h5 text-uppercase">Scope Evaluation and Requirement Definition</h3>
                                                <p class="mb-0">Initiate projects by evaluating scope and defining key requirements, tailoring models to specific client needs, irrespective of industry.                                                </p>
                                            </li>
                                            <li class="mb-1-6">
                                                <span class="text-primary text-uppercase font-weight-600 small">Step 2</span>
                                                <h3 class="h5 text-uppercase">Continuous Model Refinement</h3>
                                                <p class="mb-0">Continuously refine demand and capacity models, incorporating the latest advancements for enhanced accuracy, emphasizing iterative improvements.</p>
                                            </li>
                                            <li class="mb-1-6">
                                                <span class="text-primary text-uppercase font-weight-600 small">Step 3</span>
                                                <h3 class="h5 text-uppercase">Client-Centric Collaboration</h3>
                                                <p class="mb-0">Maintain close collaboration with clients throughout the project lifecycle, ensuring models align with their unique requirements and evolve with changing needs.</p>
                                            </li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
        <?php include 'footer.php'; ?>